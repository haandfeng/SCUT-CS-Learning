...
dfs_solver::dfs_solver(int start[3][3],int ix , int iy)
{
...
void dfs_solver::Solve(){
    ...
    while(!pq.empty()){
        ...
        for (int i =0;i<4;i++) { // each node can have 4 children (maximum)
            ...
            if(isSafe(min->x + row_dfs[i],min->y + col_dfs[i])){

                // create child and calc its cost
                Node* child = newNode(min->mat,min->x,min->y,min->x + row_dfs[i],min->y+col_dfs[i],min->level+1,min);
